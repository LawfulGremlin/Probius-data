...
void ParticleVertexMain( in Input vertIn, out VertexTransport vertOut ) {
	...
	vInputSize.w = saturate(vInputSize.w);
    ...
    if( b_iInstanceType == PARTICLE_SINGLE_AXIS ) {
        ...
        // Build base
        float3 vForward = SafeNormalize(vDelta, float3(1,0,0));
        ...
        float3 vOffset = vertIn.vOffset.x * vSizeAndAge.xyz * vRight * endScale + 0.5f * vertIn.vOffset.y * vDelta;
    ...
    GenInterpolant( WorldPos, float4( vertIn.vPosition.xyz, 1.0 ) );
