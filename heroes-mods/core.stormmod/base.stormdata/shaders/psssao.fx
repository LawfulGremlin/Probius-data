...
HALF4 PostProcessSSAO( VertexTransport vertOut ) {
    ...
    // Grab worldpos to avoid screenspace noise pattern when scrolling
    float3 vWorldPos;
    {
        float4 vCameraViewPos;
        if ( VPOS_SEMANTIC ) {
            vCameraViewPos.xz = INTERPOLANT_ScreenPos.xy * p_vUVToViewPos.xy + p_vUVToViewPos.zw;
        } else {
            float2 vUV = GetBackBufferUV( vertOut, false, false );
            vCameraViewPos.xz = vUV.xy * p_vUVToViewPos.xy + p_vUVToViewPos.zw;
        }
        vCameraViewPos.yw = float2( PIXEL_DEPTH, 1.0f );
        vCameraViewPos.xz = vCameraViewPos.xz * (PIXEL_DEPTH * p_mProjTransform[1][3] + p_mProjTransform[3][3]);
        vWorldPos = mul( p_mInvViewTransform, vCameraViewPos ).xyz;
    }
    ...
    */
    ...
    float3 vRandomNormal = ( normalize( sample2D( p_sSSAONoise, float2(dot(vWorldPos, float3(1.0, 1.0, 1.0)), dot(vWorldPos, float3(-1.0, 1.0, 1.0))) *  p_vSrcSize / 6000.0f ).xyz * 2.0f - 1.0f ) );
