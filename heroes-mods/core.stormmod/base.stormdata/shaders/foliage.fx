...
#if ( VERTEX_SHADER_VERSION >= SHADER_VERSION_VS_40 )
    uint4   vBlendIndices   : BLENDINDICES_;
#else
...
void FoliageVertexMain( in Input vertIn, out VertexTransport vertOut ) {
    ...
    HALF3 binormal = cross( vertIn.vNormal.xyz, vertIn.vTangent.xyz );
#if defined(COMPILING_SHADER_FOR_OPENGL) || defined(COMPILING_SHADER_FOR_METAL)
    ...
    DecompressRightHandedBasis( vertIn.vNormal.xyz, vertIn.vTangent.xyz, binormal );
